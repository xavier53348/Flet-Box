...
def screen_manager(
    ...
    if add_screen:
        ...
        tmp_new_phone = (
            tree_view_new_phone.build()
        )  #: ADD SCREEN PHONE IN DICT all_screens_in_app
        row_box_content_phone.controls.append(
            tmp_new_phone
        )  #: ADD PHONE inside row_box_content_phone
        ...
        #                              screen_name
        GLOBAL_VAR(
            set_global_var={tmp_new_screen_name: tmp_new_phone}
        )  # NECESARY TO VISIBLE ON OFF
        ...
        old_selected_screen.visible, new_selected_screen.visible = (
            new_selected_screen.visible,
            old_selected_screen.visible,
        )
        ...
        # {'_421': {'Avatar: 1': Container(tooltip='Avatar: 1',
        #  bgcolor='transparent', ink=True, inkcolor='red', onclick=True,
        #  onhover=True, n='content',
        #  border='{"l":{"w":0,"c":"transparent"},"t":{"w":0,"c":"transparent"},
        # "r":{"w":0,"c":"transparent"},"b":{"w":0,"c":"transparent"}}',
        # margin='{"l":0,"t":0,"r":0,"b":0}', padding='{"l":6,"t":6,"r":6,"b":6}',
        # alignment='{"x":0,"y":0}')}}

        update_all_screen: dict = {}
        secundary_update: dict = {id_main_screen: {}}
...
CURRENT_PATH: str = os.path.join("./flet_box/extra_utils/screen_manager",
                                "screens.js")
...
class NameScreen(ft.Container):
    ...
    def add_screen_widget(
        self, check_data=False, screen_name: str = "", description: str = ""
    ):
        if screen_name:
        ...
        if check_data:
            ...
            if self.widget_name:
                ...
                description = (
                    "description"
                    if self.widget_value == None
                    else self.widget_value.get("value")
                )
                if description == "":
