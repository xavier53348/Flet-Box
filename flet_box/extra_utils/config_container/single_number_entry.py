...
class SingleNumeberEntry(ft.Stack):
    ...
    def __init__(self,config_widget='max_lines',widget='',id_name_widget_dict=None):
        ...
        self.personal_configuration: dict = {
                                            ...
                                            "spread_radius"     : {"minimum"  : 0, "maximum"  : 100, "division" : 100, "value"   : 0 },
    ...
    def modify_widget_attributes(self,slider_value,slider_widget):
        ...
        if self.widget_name == "scale":              self.widget.scale             = 1 - slider_value
        if self.widget_name == "aspect_ratio":       self.widget.aspect_ratio      = 1 - slider_value
        if self.widget_name == "opacity":            self.widget.opacity           = 1 - slider_value
        if self.widget_name == "child_aspect_ratio": self.widget.child_aspect_ratio= 1 - slider_value

        if self.widget_name == "spread_radius":      self.widget.spread_radius     = slider_value
        if self.widget_name == "size":               self.widget.size              = slider_value
        if self.widget_name == "elevation":          self.widget.elevation         = slider_value
        if self.widget_name == "runs_count":         self.widget.runs_count        = slider_value
        if self.widget_name == "run_spacing":        self.widget.run_spacing       = slider_value
        if self.widget_name == "spacing":            self.widget.spacing           = slider_value
        if self.widget_name == "max_extent":         self.widget.max_extent        = slider_value
        if self.widget_name == "min_lines":          self.widget.min_lines         = slider_value
        if self.widget_name == "max_lines":          self.widget.max_lines         = slider_value
        if self.widget_name == "border_width":       self.widget.border_width      = slider_value
        if self.widget_name == "text_size":          self.widget.text_size         = slider_value
        if self.widget_name == "rotate":             self.widget.rotate           = slider_value
