...
class Gpt_text(ft.Stack):
    def __init__(self, input_text="Erase this test", color_chat="#0a1618", ask=False):
        ...
        self.icon_container = ft.Container(
            ...
            height=30,
            offset=(0.3, 0),
            content=ft.Icon(color=ft.colors.WHITE, size=18, name=ft.icons.COPY),
            on_click=lambda _: print(_),
        )  # <=== NOTE COMA
        self.widget_to_select = {
            "gpt": ft.TextField(
        ...
        self.ask_response = (
            self.widget_to_select.get("gpt")
            if not ask
            else self.widget_to_select.get("ask")
        )
...
"""
                        ...
                        content=ft.Row(
                            ...
                            controls=[
                                ...
                                ft.Container(
                                    ...
                                    height=42,
                                    width=42,
                                    on_click=lambda _: self.search_gpt_question(
                                        self.text_field_chat_gpt
    ...
    def search_gpt_question(self, e):
        ...
        if message_returned:
            self.response_question_container = Gpt_text(
                input_text=message_returned, color_chat=ft.colors.BLACK12, ask=False
            )
            self.Search_Gridwiew.controls.append(self.response_question_container)
            self.Search_Gridwiew.scroll_to(
                key=f"{len(self.Search_Gridwiew.controls)}", duration=1000
            )
            self.Search_Gridwiew.update()
